...
void timerSetDevice(struct ts_device *p_device) {
    ...
    l_driver->a_setFrequency(s_timerDevice, C_TIMER_FREQUENCY);
}
...
uint64_t timerGetTime(void) {
    ...
    return l_driver->a_getTime(s_timerDevice);
}
...
void timerSleep(uint64_t p_milliseconds) {
    ...
    uint64_t l_sleepEndTime =
        l_driver->a_getTime(s_timerDevice) + p_milliseconds + C_TIMER_PERIOD;

    while(l_driver->a_getTime(s_timerDevice) < l_sleepEndTime) {
        archHalt();
    }
}
