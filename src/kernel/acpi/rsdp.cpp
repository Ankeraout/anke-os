...
namespace kernel {
    ...
    bool rsdp_validate(const rsdp_t *rsdp) {
        // Check signature
        if(std::memcmp(&rsdp->rsdp1.signature, "RSD PTR ", 8) != 0) {
            return false;
        }
    ...
    const rsdp_t *rsdp_find_ebda() {
        ...
        uint16_t ebdaRealModeSegment = *((uint16_t *)((size_t)ram + 0x0000040e));
        uint32_t ebdaAddress = ebdaRealModeSegment << 4;

        vmm_free(ram);
        
        // Map first 4 KiB of EBDA
        const uint8_t *ebda = (const uint8_t *)vmm_map((void *)ebdaAddress, 1, true);
    ...
    const rsdp_t *rsdp_find_e0000() {
        ...
        for(size_t i = 0; i < 0x20000; i += 16) {
            if(rsdp_validate((const rsdp_t *)((size_t)ram + i))) {
                ...
                if(!ram) {
                    ...
                    return (rsdp_t *)((size_t)ram + rsdpPageOffset);
                }
            }
        }
