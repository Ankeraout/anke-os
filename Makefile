...
KERNEL_SOURCES_C+= \
	...
	$(SRCDIR)/kernel/dev/net/eth.c \
	$(SRCDIR)/kernel/dev/net/netif.c \
...
QEMUFLAGS=-m 32 -serial stdio -d cpu_reset -netdev user,id=u1 -device rtl8139,netdev=u1 -object filter-dump,id=f1,netdev=u1,file=dump.dat -hda bochs_hdd.img
ISO=anke-os.iso
